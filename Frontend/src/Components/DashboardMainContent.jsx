...
const DashboardMainContent = () => {
    return (
        <div className="h-screen w-full flex flex-col p-4 box-border">
            ...
            {/* Main content + Activity chart section */}
            <div className="flex flex-col grow gap-4 overflow-hidden">
                ...
                <div className="flex flex-col md:flex-row gap-4 h-1/2">
                    ...
                    {/* Organ Cards */}
                    <div className="w-full md:w-1/2 flex flex-col gap-3 p-2">
                        {[
                            { icon: lung, label: 'Lung', color: 'bg-red-500' },
                            { icon: teeth, label: 'Teeth', color: 'bg-cyan-500' },
                            { icon: bone, label: 'Bone', color: 'bg-cyan-500' }
                        ].map((item, idx) => (
                            <div key={idx} className="bg-blue-50 rounded-2xl flex flex-col items-center p-2 shadow">
                                ...
                                <div className="w-3/4 bg-gray-200 rounded-full h-2">
                                    <div className={`${item.color} h-2 rounded-full`} style={{ width: '25%' }} />
                                </div>
                            </div>
                        ))}
                    </div>
